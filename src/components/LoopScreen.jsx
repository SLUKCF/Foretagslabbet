...
export default function LoopScreen({ onContinue }) {
  ...
  return (
    ...
    >
      <h1 className="text-[10vw] font-extrabold text-white drop-shadow-md mb-6 leading-tight">
        Företagslabbet
      </h1>
      <p className="text-[1.5vw] mb-8">
      ...
      <div className={`transition-opacity duration-1000 ${fade ? "opacity-100" : "opacity-0"}`}>
        <p className="text-[2.2vw] italic font-semibold mt-6 mb-4 h-[3vw] max-w-[90vw]">
          {activities[index]}
        </p>
      </div>
      <div className="flex justify-center gap-2">
        {activities.map((_, i) => (
          <span key={i} className={`text-[1.2vw] ${i === index ? "text-white" : "text-white/50"}`}>
            {i === index ? "●" : "○"}
          </span>
        ))}
      </div>
      
      <p className="absolute bottom-4 text-sm text-white/40 italic">
